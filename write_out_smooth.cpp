    	...
    	for (int j = 0; j < NY; j++){
    		for (int k = 0; k < NX; k++){
    			...
    			if ((i > 0) && (i < (NZ-1)) && (j > 0) && ((j < NY-1)) && (k > 0) && ((k < NX-1))){
    				temp_t = array(k,j,i+1);
    				temp_b = array(k,j,i-1);
    				temp_u = array(k,j+1,i);
    				temp_d = array(k,j-1,i);
    				temp_r = array(k+1,j,i);
    				temp_l = array(k-1,j,i);
    				temp_c = (temp_c + temp_u + temp_d + temp_l + temp_r + temp_t + temp_b)/7.0;
    				}
    			value[i][j][k] = temp_c;
